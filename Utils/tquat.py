...
def inv(q, device):
    return torch.as_tensor([1, -1, -1, -1], dtype=torch.float32).to(device) * q
...
def ik(grot, gpos, parents, device):
    return (
        torch.cat([
            grot[..., :1, :],
            mul(inv(grot[..., parents[1:], :], device), grot[..., 1:, :]),
        ], dim=-2),
        torch.cat([
            gpos[..., :1, :],
            mul_vec(
                inv(grot[..., parents[1:], :], device),
